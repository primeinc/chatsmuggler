...
- `noUncheckedIndexedAccess` + `exactOptionalPropertyTypes` + `noImplicitOverride` — 0/25 repos *effectively* enable these (refined-github inherits via `@sindresorhus/tsconfig` but explicitly disables `noUncheckedIndexedAccess`). We will enable all three. See config-analysis.md Section 1.2 and 8.2 for exact data.
